...
namespace RedisConsoleClientTest
{
    ...
    internal static class CachedHelper
    {
        ...
        static CachedHelper()
        {
            var redisHost = System.Configuration.ConfigurationManager.AppSettings["redis"];
       ...
       private static T Base<T>(string key, Func<IRedisClient, T> func, string errorTag, string hashKey = null)
        {
            ...
            if (pool != null)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    //Logger.Log(typeof(CachedHelper), msg, ex);
                }
            }
            return default(T);
        }
       ...
       private static void Base(string key, Action<IRedisClient> action, string errorTag, string hashKey = null)
       {
        ...
        {
            Base(key, r => r.Increment(key, 1), "Increment", hashKey);
        }
        ...
        private static object BytesToObject(string key, Byte[] bytes, string hashKey)
        {
            ...
            catch (Exception ex)
            {
                Remove(key, hashKey);
                hashKey = hashKey ?? key;
                string msg = string.Format("{0}:{1}发生异常!{2} {3}", "cache", "解压", key,
                    hashKey.RedisIndex());
                //Logger.Log(typeof(CachedHelper), msg, ex);
            }
            return o;
