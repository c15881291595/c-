    ...
    public static class CacheHelper
    {
        ...
        private static T Base<T>(string key, Func<IRedisClient, T> func, string errorTag, string hashKey = null)
        {
            ...
            if (pool != null)
            {
                ...
                {
                    string msg = string.Format("{0}:{1}发生异常!{2},异常日志:{3}", "cache", errorTag, key, ex.ToString());
        ...
        public static List<string> GetAllKey()
        {
            ...
            for (int i = 0; i < readWriteHosts.Length; i++)
            {
                using (var r = pool.GetClient(i))
                {
                    if (r != null)
                    {
                        stringList.AddRange(r.GetAllKeys() ?? new List<string>());
                    }
                }
            }
            return stringList;
        ...
        /// <param name="key"></param>
        /// <param name="hashKey"></param>
        public static void Remove(string LinkKey)
        {
            for (int i = 0; i < readWriteHosts.Length; i++)
            {
                using (var r = pool.GetClient(i))
                {
                    if (r != null)
                    {
                        //List<string> str = r.GetAllKeys() ?? new List<string>();
                        //str = str.Where(n => n.StartsWith(LinkKey)).ToList();
                        //r.RemoveAll(str);
                        List<string> str = r.SearchKeys(LinkKey + "*") ?? new List<string>();
                        if (str.Count > 0)
                        {
                            r.RemoveAll(str);
                        }
        ...
        public static void RemoveByKeyList(List<string> keyList)
        {
            if (keyList == null || keyList.Count <= 0) throw new Exception("listkey as NullOrEmpty");
            for (int i = 0; i < readWriteHosts.Length; i++)
            {
                using (var r = pool.GetClient(i))
                {
